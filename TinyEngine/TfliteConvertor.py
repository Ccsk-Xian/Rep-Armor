...
class TfliteConvertor(object):
    ...
    def parseOperatorInfo(self):
        ...
        for i in range(operators_len):
            ...
            # print("***")
            # print(getOpCodeStr(op, self.model))  trans-pad-trans
            if i + 2 < operators_len - 2:
                ...
                if self.checkIfRisCompatibility(three_op_sequence):
                    logging.info("erase trans-pad-trans")
                    skip_next_ops = 2
                    input_tensors = get_input_tensors(op, self.model)
        
                    input_tensor = input_tensors[0]
                    # print("***")
                    # print(input_tensor.tensor_idx)

                    output_tensors = get_output_tensors(next_next_op, self.model)
                ...
                if self.checkDoubleTrans(three_op_sequence):
                    logging.info("erase trans-trans")
                    skip_next_ops = 1
                    continue
    ...
    def checkIfRisCompatibility(self, three_op_sequence):
        if (
            getOpCodeStr(three_op_sequence[0], self.model) == "TRANSPOSE"
            and getOpCodeStr(three_op_sequence[1], self.model) == "PAD"
            and getOpCodeStr(three_op_sequence[2], self.model) == "TRANSPOSE"
        ):
            return True
        return False
    
    def checkDoubleTrans(self, three_op_sequence):
        if (
            getOpCodeStr(three_op_sequence[0], self.model) == "TRANSPOSE"
            and getOpCodeStr(three_op_sequence[1], self.model) == "TRANSPOSE"
