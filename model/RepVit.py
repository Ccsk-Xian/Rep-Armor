...
class RepViT(nn.Module):
    def __init__(self, cfgs, num_classes=1000, distillation=False ,mul_width=1.):
        super(RepViT, self).__init__()
        # setting of inverted residual blocks
        self.cfgs = cfgs

        # building first layer
        input_channel = int(self.cfgs[0][2]*mul_width)
        ...
        for k, t, c, use_se, use_hs, s in self.cfgs:
            output_channel = _make_divisible(int(c*mul_width), 8)
...
@register_model
def repvit_m0_6(pretrained=False, num_classes = 1000, distillation=True, mul_width=0.6):
    """
    ...
    return RepViT(cfgs, num_classes=num_classes, distillation=distillation, mul_width=mul_width)
