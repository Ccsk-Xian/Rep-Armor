...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.nodown_orchange = self.stride == 1 and inp == oup

        if self.nodown_orchange:
    ...
    def forward(self, x):
        if self.nodown_orchange:
            out = self.token_mixer(x)+x

            return out + self.channel_mixer(out)
        else:
            out = self.token_mixer(x)
            return self.channel_mixer(out)+out
...
def mobilemetanet(num_classes):
    return mobilenetv2_T_w(2, 0.62, num_classes)

from fvcore.nn import FlopCountAnalysis, parameter_count_table
if __name__ == '__main__':
    x = torch.randn(2, 3, 224, 224)

    net = mobilemetanet(100)
